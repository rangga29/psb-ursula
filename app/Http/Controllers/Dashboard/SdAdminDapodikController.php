...
class SdAdminDapodikController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:sd-dapodik-list|sd-dapodik-edit|sd-dapodik-verification|sd-dapodik-report')->only('index', 'show');
        $this->middleware('permission:sd-dapodik-edit')->only('editDataPribadi', 'updateDataPribadi', 'editDataKependudukan', 'updateDataKependudukan', 'editDataAyah', 'updateDataAyah', 'editDataIbu', 'updateDataIbu', 'editDatawali', 'updateDatawali');
        $this->middleware('permission:sd-dapodik-report')->only('singlePdfReport');
        $this->middleware('permission:sd-dapodik-verification')->only('dapodikVerification');
    }
    ...
    public function index()
    {
        ...
        $all_dapodik = SdRegistration::where('approval_status', 1)->orderBy('registration_code', 'asc')->get();
        $not_have_dapodik = SdRegistration::where('approval_status', 1)->where('dapodik_status', 0)->orderBy('registration_code', 'asc')->get();
        $not_verified_dapodik = SdRegistration::where('approval_status', 1)->where('dapodik_status', 1)->orderBy('registration_code', 'asc')->get();
        $verified_dapodik = SdRegistration::where('approval_status', 1)->where('dapodik_status', 2)->orderBy('registration_code', 'asc')->get();
    ...
    public function editDataPribadi(SdRegistration $sd_registration)
    {
        $page_setting = [
            'unit' => 'PSB',
            'unit_slug' => 'psb',
        ...
        if ($dapodik_pribadi == null) {
            return redirect()->route('dashboard.sd.admin.dapodik.show')->with('error', 'Data Siswa - Data Pribadi Tidak Ditemukan');
        }

        return view('backend.dapodik.admin.editDataPribadi', compact('page_setting', 'registration_data', 'dapodik_pribadi'));
    }

    public function updateDataPribadi(DapodikPribadiRequest $request, SdRegistration $sd_registration)
    {
        ...
        DapodikPribadi::where('user_id', $sd_registration->user_id)->update($validateData);
        return redirect()->route('dashboard.sd.admin.dapodik.show', $sd_registration->unique_code)->with('success', 'Data Siswa - Data Pribadi Berhasil Diubah');
    }

    public function editDataKependudukan(SdRegistration $sd_registration)
    {
        $page_setting = [
            'unit' => 'PSB',
            'unit_slug' => 'psb',
        ...
        if ($dapodik_kependudukan == null) {
            return redirect()->route('dashboard.sd.admin.dapodik.show')->with('error', 'Data Siswa - Data Kependudukan Tidak Ditemukan');
        }

        return view('backend.dapodik.admin.editDataKependudukan', compact('page_setting', 'registration_data', 'dapodik_kependudukan'));
    }

    public function updateDataKependudukan(UpdateDapodikKependudukanRequest $request, SdRegistration $sd_registration)
    {
        $validateData = $request->validated();
        ...
        if ($request->file('pas_foto')) {
            if ($request->oldPasFoto && File::exists('upload/pas_foto/sd/' . $request->oldPasFoto)) {
                File::delete('upload/pas_foto/sd/' . $request->oldPasFoto);
            }
            $name = $sd_registration->registration_code . '_' . Str::random(20) . '.' . $request->file('pas_foto')->extension();
            $request->file('pas_foto')->move('upload/pas_foto/sd', $name);
            $validateData['pas_foto'] = $name;
        }

        DapodikKependudukan::where('user_id', $sd_registration->user_id)->update($validateData);
        return redirect()->route('dashboard.sd.admin.dapodik.show', $sd_registration->unique_code)->with('success', 'Data Siswa - Data Kependudukan Berhasil Diubah');
    }

    public function editDataAyah(SdRegistration $sd_registration)
    {
        $page_setting = [
            'unit' => 'PSB',
            'unit_slug' => 'psb',
        ...
        if ($dapodik_ayah == null) {
            return redirect()->route('dashboard.sd.admin.dapodik.show')->with('error', 'Data Siswa - Data Ayah Tidak Ditemukan');
        }

        return view('backend.dapodik.admin.editDataAyah', compact('page_setting', 'registration_data', 'dapodik_ayah'));
    }

    public function updateDataAyah(DapodikAyahRequest $request, SdRegistration $sd_registration)
    {
        $validateData = $request->validated();
        ...
        $validateData['ayah_nama_lengkap'] = Str::title($validateData['ayah_nama_lengkap']);
        DapodikAyah::where('user_id', $sd_registration->user_id)->update($validateData);
        return redirect()->route('dashboard.sd.admin.dapodik.show', $sd_registration->unique_code)->with('success', 'Data Siswa - Data Ayah Berhasil Diubah');
    }

    public function editDataIbu(SdRegistration $sd_registration)
    {
        $page_setting = [
            'unit' => 'PSB',
            'unit_slug' => 'psb',
        ...
        $dapodik_ibu = DapodikIbu::where('user_id', $registration_data->user_id)->first();
        if ($dapodik_ibu == null) {
            return redirect()->route('dashboard.sd.admin.dapodik.show')->with('error', 'Data Siswa - Data Ibu Tidak Ditemukan');
        }

        return view('backend.dapodik.admin.editDataIbu', compact('page_setting', 'registration_data', 'dapodik_ibu'));
    }

    public function updateDataIbu(DapodikIbuRequest $request, SdRegistration $sd_registration)
    {
        $validateData = $request->validated();
        ...
        $validateData['ibu_nama_lengkap'] = Str::title($validateData['ibu_nama_lengkap']);
        DapodikIbu::where('user_id', $sd_registration->user_id)->update($validateData);
        return redirect()->route('dashboard.sd.admin.dapodik.show', $sd_registration->unique_code)->with('success', 'Data Siswa - Data Ibu Berhasil Diubah');
    }

    public function editDataWali(SdRegistration $sd_registration)
    {
        $page_setting = [
            'unit' => 'PSB',
            'unit_slug' => 'psb',
        ...
        if ($dapodik_wali == null) {
            return redirect()->route('dashboard.sd.admin.dapodik.show')->with('error', 'Data Siswa - Data Wali Tidak Ditemukan');
        }

        return view('backend.dapodik.admin.editDataWali', compact('page_setting', 'registration_data', 'dapodik_wali'));
    }

    public function updateDataWali(DapodikWaliRequest $request, SdRegistration $sd_registration)
    {
        $validateData = $request->validated();
        ...
        $validateData['wali_nama_lengkap'] = Str::title($validateData['wali_nama_lengkap']);
        DapodikWali::where('user_id', $sd_registration->user_id)->update($validateData);
        return redirect()->route('dashboard.sd.admin.dapodik.show', $sd_registration->unique_code)->with('success', 'Data Siswa - Data Wali Berhasil Diubah');
    }
    ...
    public function singlePdfReport(SdRegistration $sd_registration)
    {
        ...
        $pdf->loadview('backend.dapodik.admin.singlePdf', [
            ...
            'registration_data' => $sd_registration,
            'dapodik_pribadi' => DapodikPribadi::where('user_id', $sd_registration->user_id)->first(),
            'dapodik_kependudukan' => DapodikKependudukan::where('user_id', $sd_registration->user_id)->first(),
            'dapodik_ayah' => DapodikAyah::where('user_id', $sd_registration->user_id)->first(),
            'dapodik_ibu' => DapodikIbu::where('user_id', $sd_registration->user_id)->first(),
            'dapodik_wali' => DapodikWali::where('user_id', $sd_registration->user_id)->first(),
        ])->setPaper('a4');
        return $pdf->stream('[' . Carbon::now()->format('Ymd') . '] PSB SD - Data Siswa - ' . $sd_registration->full_name . '.pdf');
    }

    public function dapodikVerification(SdRegistration $sd_registration, $code)
    {
        SdRegistration::where('id', $sd_registration->id)->update(['dapodik_status' => $code]);
        ...
        return redirect()->route('dashboard.sd.admin.dapodik.index')->with('success', $message);
    }
}
